...
export function ContactCard({ contact, onUpdate, onSelectContact, onDelete }: ContactCardProps) {
  ...
  useEffect(() => {
    const runAutoGenerate = async () => {
      // Skip if already auto-generated for this contact, or settings not loaded
      if (!autoGenSettings || !autoGenSettings.enabled) return;
      if (hasAutoGeneratedRef.current === contact.id) return;
      ...
      if (hasExistingData) {
        hasAutoGeneratedRef.current = contact.id;
        return;
      }

      // Mark as in-progress for this contact
      hasAutoGeneratedRef.current = contact.id;
    ...
    // Small delay to ensure AI data is loaded first
    const timer = setTimeout(runAutoGenerate, 500);
    return () => clearTimeout(timer);
  }, [contact.id, autoGenSettings, companyAI.ai_summary, contactPersona, aiScript]);
