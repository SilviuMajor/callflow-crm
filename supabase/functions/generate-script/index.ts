...
serve(async (req) => {
  ...
  try {
    ...
    const { contact_id }: GenerateScriptRequest = await req.json();
    console.log(`Generate script for contact: ${contact_id}`);
    ...
    const { data: scriptTemplate, error: scriptError } = await supabase
      .from('ai_scripts')
      .select('*')
      .limit(1)
      .single();

    if (scriptError || !scriptTemplate) {
